document.addEventListener('DOMContentLoaded', function() {
    ...
    function fillContentFromConfig() {
        ...
        // Bagian Our Story: GENERATE DINAMIS GAMBAR STORY
        const storyTimelineContainer = document.querySelector('.story-timeline');
        if (storyTimelineContainer && weddingConfig.story && Array.isArray(weddingConfig.story) && weddingConfig.story.length > 0) {
            storyTimelineContainer.innerHTML = ''; // Hapus semua timeline-item yang ada untuk regenerasi
            weddingConfig.story.forEach((item, index) => {
                const timelineItemDiv = document.createElement('div');
                timelineItemDiv.classList.add('timeline-item');
                timelineItemDiv.classList.add(index % 2 === 0 ? 'left-animate' : 'right-animate');
                ...
                imgElement.classList.add('story-photo');
                imgElement.alt = `Our Story ${index + 1}`;
                if (weddingConfig.images.story && Array.isArray(weddingConfig.images.story) && weddingConfig.images.story[index]) {
                    imgElement.src = weddingConfig.images.story[index];
                }
                timelineItemDiv.appendChild(imgElement);

                const descDiv = document.createElement('div');
                descDiv.classList.add('timeline-description');
                descDiv.textContent = item.description;
                timelineItemDiv.appendChild(descDiv);

                storyTimelineContainer.appendChild(timelineItemDiv);
            });
        } else {
            // Jika tidak ada cerita atau cerita kosong, sembunyikan section ini
        ...
        if (bankAccountsContainer) {
            ...
            bankAccounts.forEach((bank, index) => {
                ...
                bankAccountDiv.innerHTML = `
                    <p><strong>Bank ${bank.name}</strong></p>
